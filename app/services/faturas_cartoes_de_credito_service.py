...
class FaturasCartoesDeCreditoService:
    ...
    def get_all_faturas(self) -> List[FaturasCartoesDeCreditoModel]:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                rows = cur.fetchall()
                return [
                    self.faturas_cartoes_de_credito_model.from_dict(row) for row in rows
                ]

    def get_fatura_by_id(
        self, id_fatura: int
    ) -> Optional[FaturasCartoesDeCreditoModel]:
        """Retorna uma fatura específica por ID."""
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'SELECT * FROM "{self.table_name}" WHERE "id_fatura_cartao_credito" = %s',
                    (id_fatura,),
                )
                row = cur.fetchone()
                return (
                    self.faturas_cartoes_de_credito_model.from_dict(row)
                    if row
                    else None
                )

    def get_faturas_by_cartao(
        self, id_cartao: int
    ) -> List[FaturasCartoesDeCreditoModel]:
        """Retorna todas as faturas de um cartão."""
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'SELECT * FROM "{self.table_name}" WHERE "id_cartao" = %s',
                    (id_cartao,),
                )
                rows = cur.fetchall()
                return [
    ...
    def insert_fatura(
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                result = cur.fetchone()
                result_model = (
                    self.faturas_cartoes_de_credito_model.from_dict(result)
    ...
    def delete_fatura(self, id_fatura: int) -> bool:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'DELETE FROM "{self.table_name}" WHERE "id_fatura_cartao_credito" = %s',
                    (id_fatura,),
                )
                return cur.rowcount > 0
