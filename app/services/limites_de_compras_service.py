...
class LimitesDeComprasService:
    ...
    def get_limite_by_id(self, id_limite: int) -> Optional[LimitesDeComprasModel]:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'SELECT * FROM "{self.table_name}" WHERE "id_limite_compra" = %s',
                    (id_limite,),
                )
                row = cur.fetchone()
                return self.limites_de_compras_model.from_dict(row) if row else None

    def get_limite_by_categoria(
        self, id_categoria: int
    ) -> Optional[LimitesDeComprasModel]:
    ...
    def insert_limite(self, id_categoria: int, limite_categoria: float) -> int:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                ...
                result = cur.fetchone()
                result_model = (
                    self.limites_de_compras_model.from_dict(result)
    ...
    def update_limite(
        self, id_limite: int, id_categoria: int = None, limite_categoria: float = None
    ) -> bool:
    ...
    def delete_limite(self, id_limite: int) -> bool:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'DELETE FROM "{self.table_name}" WHERE "id_limite_compra" = %s',
                    (id_limite,),
                )
                return cur.rowcount > 0
