...
class SaidasRealizadasService:
    ...
    def get_saida_realizada_by_id(
        self, id_saida: int
    ) -> Optional[SaidasRealizadasModel]:
        """Retorna uma saída realizada específica por ID."""
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'SELECT * FROM "{self.table_name}" WHERE "id_saida" = %s',
                    (id_saida,),
                )
                row = cur.fetchone()
                return SaidasRealizadasModel.from_dict(row) if row else None

    def insert_saida_realizada(
        self,
        id_categoria: int,
        id_banco: int,
        data_saida: datetime,
    ...
    def update_saida_realizada(
        ...
        query = (
            f'UPDATE "{self.table_name}" SET {", ".join(updates)} WHERE "id_saida" = %s'
        )
    ...
    def delete_saida_realizada(self, id_saida: int) -> bool:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'DELETE FROM "{self.table_name}" WHERE "id_saida" = %s',
                    (id_saida,),
                )
                return cur.rowcount > 0
