...
class BancosService:
    ...
    def get_banco_by_id(self, id_banco: int) -> Optional[BancosModel]:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'SELECT * FROM "{self.table_name}" WHERE "id_banco" = %s',
                    (id_banco,),
                )
                row = cur.fetchone()
    ...
    def insert_banco(
        self, nome_banco: str, valor_em_conta: float = 0.0, valor_investido: float = 0.0
    ) -> int:
    ...
    def update_banco(
        ...
        query = (
            f'UPDATE "{self.table_name}" SET {", ".join(updates)} WHERE "id_banco" = %s'
        )
    ...
    def delete_banco(self, id_banco: int) -> bool:
        ...
        with self.postgres_service.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f'DELETE FROM "{self.table_name}" WHERE "id_banco" = %s',
                    (id_banco,),
                )
                return cur.rowcount > 0
