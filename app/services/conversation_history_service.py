...
class ConversationHistoryService:
    ...
    def save_message(
        self, numero_telefone: str, tipo_mensageiro: str, conteudo_mensagem: str
    ) -> bool:
        """
        ...
        """
        try:
            encrypted_conteudo_mensagem = self._encrypt_conteudo_mensagem(
                conteudo_mensagem
            )
    ...
    def get_history(
        self, numero_telefone: str, limit: int = 10, hours_back: Optional[int] = 24
    ) -> List[Message]:
        """
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                for row in rows:
                    try:
                        decrypted_conteudo_mensagem = self._decrypt_conteudo_mensagem(
                            row["conteudo_mensagem"]
                        )

                        # Ajusta o tipo_mensageiro para o formato esperado pelo sistema
                        tipo_mensageiro = (
                            "user" if row["tipo_mensageiro"] == "user" else "bot"
                        )
                        ...
                        log_service.error(
                            f"Erro ao descriptografar mensagem: {e}", exc_info=True
                        )
                        continue
                ...
                log_service.info(
                    f"Recuperadas {len(messages)} mensagens para {numero_telefone}"
                )
                return messages
